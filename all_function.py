...
def count_plot_top_features(data, feature, target):
    ...
    st.write(f'When output is {feature}')
    for i in column:
        plot = px.histogram(data, x=i, barmode='group')
...
def download_configs(model):
    output_model = pickle.dumps(model)
    b64 = base64.b64encode(output_model).decode()
    href = f'<a href="data:file/output_model;base64,{b64}" download="myfile.pkl">Download Configurations .pkl File</a>'
    st.markdown(href, unsafe_allow_html=True)

def file_upload():
    file = st.file_uploader('Upload a file', type = ['csv','zip'])
    if file is not None:
        file_details = {"Filename":file.name,"FileType":file.type,"FileSize":file.size}
        #st.write(file_details)
        if file_details['FileType'] == 'application/zip':
            with zipfile.ZipFile(file_details['Filename'], 'r') as zip_ref:
                file_unzipped = zip_ref.extractall()
                st.write(type(file_unzipped))
            data = pd.read_parquet(file_unzipped)
        else:
            data = pd.read_csv(file)
    data_print = data.sample(n = 100)
    st.write(data_print)
    return(data)
...
def get_table_download_link(df):
    ...
    return f'<a href="data:file/csv;base64,{b64}" download="dataset.csv">Download Top Features CSV File</a>'